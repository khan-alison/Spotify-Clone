...
export default function PlaylistDetails(props: IUserPlaylist) {
    ...
    spotifyApi.getPlaylist(playListId.playlistID).then((playList: any) => {
      ...
      spotifyApi
        .getRecommendations({
          min_energy: 0.4,
          seed_artists: [
            playList?.body?.tracks?.items[0]?.track?.artists[0]?.id,
            playList?.body?.tracks?.items[1]?.track?.artists[0]?.id,
          ],
          min_popularity: 50,
        })
        .then(
  ...
  useEffect(() => {
    let handleScroll = () => {
      if (window.pageYOffset == 0 && window.pageYOffset < 336) {
        ...
        window.pageYOffset >= 336 &&
        window.pageYOffset <= clientHeight + offsetTop - 180
      ) {
        setPosition("sticky");
  ...
  return (
    <div className={style.container}>
      <div
        style={{ backgroundColor: `${backgroundColor}` || "#444544" }}
      ...
      >
        ...
        <div className={style.headerContent}>
          <div className={style.headerType}>{userPlaylistData?.type}</div>
          <div className={style.headerName} onClick={() => setIsOpen(true)}>
          ...
          <PopUp
            ...
            onClose={handleClose}
            currName={userPlaylistData?.name}
          />
          <div className={style.headerDescription}>
            {userPlaylistData?.description}
          </div>
          <div className={style.headerInfo}>
            <p style={{ fontWeight: "bold" }}>
              {userPlaylistData?.owner?.display_name}
            </p>
            <p style={{ fontWeight: "bold" }}>
              &bull;
              {` ${playListLength} ${
                userPlaylistData?.tracks?.total === 1 ? "song" : "songs"
              }`}
            </p>
      ...
      <div className={style.body}>
        ...
        {!findMore ? (
          <>
            ...
            <div className={style.refresh} onClick={refreshHandle}>
          ...
          <>
            ...
            <div className={style.searchContainer}>
              <div className={style.searchHeader}>
                ...
                <ClearIcon className={style.clearIcon} onClick={unfindHandle} />
              </div>
