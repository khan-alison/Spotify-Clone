...
function ArtistDetails(props: IArtistDetails) {
  ...
  useEffect(() => {
    ...
    props.spotify.isFollowingArtists([artistId]).then(
      function (data: any) {
  ...
  const handleUnfollowClick = (event: any) => {
    if (event.currentTarget.textContent == "Follow") {
      spotifyApi.followArtists([artistId]).then(
        (data: any) => {
          setIsFlw(true);
        },
        (err: any) => {
      ...
      spotifyApi.unfollowArtists([artistId]).then(
        function (data: any) {
          setIsFlw(false);
        },
        function (err: any) {
  ...
  return (
    <div className={style.container}>
      ...
      <div
        ...
        style={{ backgroundImage: `linear-gradient(${bgColor},#222221)` }}
      >
        <div className={style.artistOptions}>
          ...
          />
          <div onClick={handleUnfollowClick} className={style.flwBtn}>
            ...
            {artistTopTracks.map((track: any, index: number) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={style.popularity}>{track.popularity}</div>
                  <div className={style.actions}>
                    {isFar ? (
                      <FavoriteIcon
                        onMouseDown={() => unlikedHandle(track.id)}
                        onMouseUp={() => handleClick(track.id)}
                        className={style.farIcon}
                      />
                    ) : (
                      <FavoriteBorderIcon
                        onMouseUp={() => handleClick(track.id)}
                        onMouseDown={() => likeHandle(track.id)}
                        className={style.farIcon}
                      />
                    )}
                    <div className={style.duration_ms}>
