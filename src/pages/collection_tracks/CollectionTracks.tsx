...
import { getListUri, getUri } from "../../redux/actions/actions";
...
export default function CollectionTracks(props: ICollectionTracks) {
  ...
  const playIconClickedHandle = () => {
    const list = savedTracks.reduce((acc: any, cur: any) => {
    ...
    <div className={style.container}>
      <div className={style.header}>
        ...
        <div className={style.headerContent}>
          <div className={style.headerType}>playlist</div>
          <div className={style.headerName}>Favorite</div>
          <div className={style.headerInfo}>
            <p style={{ fontWeight: "bold" }}>
              {data?.user?.display_name} &bull;
              {` ${length} ${length === 1 ? "song" : "songs"}`}
            </p>
          </div>
        </div>
      </div>
